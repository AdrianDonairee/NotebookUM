...
def _validate_pdf_bytes(pdf_bytes: bytes) -> None:
    """
    Validate PDF bytes have correct signature.
    
    Args:
        pdf_bytes: The bytes to validate
        
    Raises:
        InvalidPDFException: If bytes are empty or don't have valid PDF signature
    """
    if not pdf_bytes:
        raise InvalidPDFException(
            detail="Cannot extract text from empty file",
            instance=_EXTRACTION_ENDPOINT
        )
